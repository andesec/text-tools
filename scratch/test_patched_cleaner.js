...
// Let's implement robustCleanJson with candidate validation
function robustCleanJsonPatched(text) {
  ...
  while (i < text.length) {
    ...
    if (ch === '"') {
      ...
      if (expectKey) {
        // For keys, always find the first candidate that is a valid key
        ...
        // Collect all candidates that pass the basic follow-up check
        let validCandidates = [];
        for (let idx = 0; idx < candidateIndices.length; idx++) {
          const qIndex = candidateIndices[idx];
          let passes = false;
        ...
        // Search from end (backwards) and validate which one results in a repairable document
        for (let idx = validCandidates.length - 1; idx >= 0; idx--) {
          ...
          // Reconstruct the remaining text and check if it's repairable
          const testRemaining = text.substring(qIndex + 1);
          // If we are testing the last candidate, or if the rest of the document is repairable
          try {
            // We append a dummy structure to make it valid JSON for testing
            const testDocument = result + '"' + escapedContent + '"' + testRemaining;
            jsonrepair.jsonrepair(testDocument);
            // If jsonrepair succeeded, this candidate is perfect!
            closingQuoteIndex = qIndex;
            break;
          } catch (e) {
            // If it failed, try the next candidate
          }
        }

        // Fallback to first valid candidate if all validation failed
...
try {
  ...
  console.log("Prompt length:", parsed.prompt.length);
} catch (e) {
