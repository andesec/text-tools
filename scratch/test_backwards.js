...
// Patch robustCleanJson to search backwards
    ...
    if (ch === '"') {
      ...
      if (expectKey) {
        for (let idx = candidateIndices.length - 1; idx >= 0; idx--) {
          ...
          // We don't have isValidFollowUp exported easily, so mock it for test
          let nextNonWs = '';
...
try {
  ...
  console.log("SAFEJSONPARSE SUCCESS!", Object.keys(result));
} catch (e) {
